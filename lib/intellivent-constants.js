...
// Recycle a healthy session before the fan tears it down itself.
//
// athombv/homey-apps-sdk-issues#454 measured that once the PERIPHERAL drops a
// GATT session, Homey's BLE manager becomes permanently unable to reconnect to
// it - app restart does not help, only a full Homey reboot. The fan drops the
// link on its own after minutes to hours regardless of keepalive traffic
// (~11 min observed on fw 13.4.0, ~87 min on 13.4.1). Disconnecting cleanly
// from our side first keeps teardown app-initiated, which is the only lever an
// app has against that failure mode.
//
// Deliberately below the shortest observed peripheral-initiated drop.
// Configurable per device; 0 disables recycling.
const DEFAULT_SESSION_RECYCLE_MINUTES = 8;
...
module.exports = {
  ...
  AUTH_REGEN_COOLDOWN,
  AUTH_RETRY_INTERVAL,
  DEFAULT_SESSION_RECYCLE_MINUTES,
};
