...
class IntelliventParser {
  ...
  static validateRpm(rpm) {
    // Math.max/min pass NaN straight through, and Buffer writes NaN as 0 -
    // a bad value must fail loudly rather than reach the fan as 0 RPM
    const value = Number(rpm);
    if (!Number.isFinite(value)) {
      throw new Error(`Invalid RPM: ${rpm}`);
    }
    return Math.max(Constants.MIN_RPM, Math.min(Constants.MAX_RPM, Math.round(value)));
  }
  ...
  static validateDetection(level) {
    ...
    if (!Number.isFinite(value)) {
      throw new Error(`Invalid detection level: ${level}`);
    }
    // Floor at 1: 0 is not a meaningful sensitivity on the wire, and legacy
    // flow arguments from v0.1.x used 0 to mean low (= wire value 1)
    return Math.max(1, Math.min(3, Math.round(value)));
  }

  /**
   * Validate time value: non-negative, and no larger than the field it is
   * written to (Buffer writes throw on overflow)
   * @param {number} time - Time value
   * @param {number} [max] - Largest value the field can hold
   * @returns {number} - Validated time value
   */
  static validateTime(time, max = 0xFF) {
  ...
  static parseSensorData(data) {
    ...
    // Calculate humidity using logarithmic formula (one decimal, as upstream)
    let humidity = 0;
    if (humidityRaw > 0) {
      humidity = Math.round(Math.log(humidityRaw / 10) * 10 * 10) / 10;
    }
  ...
  static encodeBoost(enabled, rpm, durationMinutes) {
    ...
    buffer.writeUInt8(enabled ? 1 : 0, 0);
    buffer.writeUInt16LE(this.validateRpm(rpm), 1);
    // Seconds in a uint16: at most 1092 minutes
    buffer.writeUInt16LE(this.validateTime(durationMinutes, Math.floor(0xFFFF / 60)) * 60, 3);
    return buffer;
  }
  ...
  /**
   * Compare a reported UUID against a full 128-bit constant. Homey may
   * report standard 16-bit UUIDs in short form (e.g. '2a24').
   * @param {string} reported - UUID as reported by Homey
   * @param {string} expected - Full 128-bit UUID constant
   * @returns {boolean}
   */
  static uuidMatches(reported, expected) {
    const a = String(reported).toLowerCase().replace(/-/g, '');
    const b = String(expected).toLowerCase().replace(/-/g, '');
    if (a === b) return true;
    return a.length === 4 && b === `0000${a}00001000800000805f9b34fb`;
  }

  /**
   * Decode a UTF-8 string characteristic (Device Information service)
   * @param {Buffer} data - Raw data
   * @returns {string}
   */
  static parseString(data) {
    // Some firmwares pad with NUL bytes
